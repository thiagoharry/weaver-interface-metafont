...
\secao{6. Declaração de Variáveis e as Variáveis Numéricas}
...
Quando uma variável é declarada, devemos fazer três coisas:

1) Se a variável não existir, devemos alocar uma estrutura da memória
para armazenar seu conteúdo.

2) O conteúdo da variável deve ser esvaziado e um valor padrão
representando ``valor desconhecido'' deve ser colocado.

3) Se a variável é nova e não existia antes, devemos percorrer a lista
de tokens e procurar por ocorrências desta variável mais adiante. E
devemos atualizar o ponteiro destes tokens para que apontem para a
região de memória recém-alocada para esta variável.

Uma variável numérica será representada pela seguinte estrutura:
...
struct numeric_variable{
  int type;          // Deve ser: TYPE_T_NUMERIC
  int nesting_level;
  void *next;
  float value;      // Inicializado como NaN = desconhecido
};
@
\fimcodigo
...
A estrutura da fonte tipográfica possui ponteiros para armazenar tanto
variáveis globais com seus nomews preservados, bem como armazenar sem
seus nomes. Além disso, ela possui ponteiros para as funções usadas
para alocar e desalocar tais estruturas:
