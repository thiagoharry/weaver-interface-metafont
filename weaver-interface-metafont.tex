...
\alinhaverbatim
<Expressão de Caminho> -> <Expressão de Par> | <Caminho Terciário> |
...
<Especificação de Direção> -> Vazio | { curl <Expressão Numérica> } |
                              { <Expressão de Par } |
                              { <Expressão Numérica> , <Expressão Numérica> }
...
para ela. Se temos uma expressão de caminho cíclico (o último token
é \monoespaco{cycle}), então o número de pontos que alocaremos é igual
ao número de junções. Se não for cíclico, iremos alocar igual ao
número de junções mais um:
...
if(number_of_joins == 0){
  ...
  //return eval_path_tertiary(mf, cx, begin_expression, end_expression, result);
  return false;
}
@
\fimcodigo
...
E devemos fazer o mesmo com a outra extremidade da junção: $z_2$:
...
um par especificando um vetor de direção ou podem ser um inteiro
especificando um grau de curvatura.

Vamos armazenar os especificadores, caso sejam um par, em
variáveis \monoespaco{(w0\_x, w0\_y)} e \monoespaco{(w1\_x, w1\_y)}. E
caso sejam uma curvatura de \monoespaco{curl0} e \monoespaco{curl1}.
Também vamos armazenar estes valores da iteração anterior e também da
próxima, já que em alguns casos será necessário lembrar o que havia
antes ou espiar o que existe depois para montarmos uma curva levando
em conta um contexto maior:
...
valores presentes na junção devemos reinicializar tais valores. Os
valores anteriores passam a armazenar o que antes eram os segundos
valores atuais. O primeiro dos especificadores atuais não precisa ser
lido novamente e só herda valores já armazenados se por algum motivo
ele foi lido anteriormente e está armazenado na variável que na
iteração anterior armazenou o que seria o próximo valor. Depois disso,
os próximos valores são apagados. E por fim, lemos o segundo
especificador de direção:
...
Agora checamos se temos uma especificação de direção na forma de
curvatura com um token \monoespaco{curl}:
...
float tension0 = NAN, tension1 = NAN, u_x = NAN, u_y = NAN, v_x = NAN,
      v_y = NAN;
bool atleast0, atleast1; // Se a tensão é só um limite inferior
@
\fimcodigo
