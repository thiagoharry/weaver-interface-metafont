...
The variable \monoespaco{convex} stores if our pen is convex. If so,
we can use very simple triangulation algorithms that will perform the
computations in O(n).

The variable \monoespaco{straight} stores if the pen format is
...
The variable \monoespaco{circular} stores if we know that the pen has
...
if(!pen.straight && a.value != 0.0){// Requires to retriangulate the pen:
  if(pen.referenced != NULL){
    recursive_copy_points(temporary_alloc, &(pen.format),
                          pen.referenced -> format, true);
    pen.referenced = NULL;
...
if(!pen.straight){ // Requer retriangular a caneta:
  if(pen.referenced != NULL){
    recursive_copy_points(temporary_alloc, &(pen.format),
                          pen.referenced -> format, true);
    pen.referenced = NULL;
