...
\subsubsecao{8.1.3. Módulo, Valores Intermediários,
    Funções Trigonométricas, Exponenciais, Piso e
    Valores Aleatórios Uniformes}
...
2) Se o último token for um ``]'', então temos uma construção do tipo
$a[b,c]$, onde $a$ é átomo numérico enquanto $b$ e $c$ são expressões
numéricas. Devemos então separar os tokens entre estas três partes,
descobrir o valor de cada um e calcular $a(b+c)$.
...
3) Se encontramos o operador \monoespaco{length}, apenas checamos se a
...
4) Se encontrarmos algum dos operadores numéricos, temos um operador
...
5) Nos demais casos, temos uma multiplicação escalar em que o escalar
...
if(begin_expression -> type == TYPE_NUMERIC &&
   ...
   ((struct generic_token * ) begin_expression -> next) -> type ==
   TYPE_DIVISION){
  result = ((struct numeric_token *) begin_expression) -> value /
...
@<Atribuição de Variável de Par@>=
  ...
  //ret = eval_pair_expression(mf, cx, begin_expression,
  //                           (struct generic_token *) *end_token_list,
  //                           &result);
  //if(!ret)
  //  return false;
...
% começo, ['penoffset'], 'shifted', ['point'], 'rotated', 'scaled'
% ['precontrol'], ['of'], ['postcontrol'], 'slanted', 'xscaled',
% 'yscaled, 'zscaled', 'intersectiontimes'
