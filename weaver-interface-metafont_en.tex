...
@<Path Expression: Iterating over Joins@>=
{
  ...
  if(end_expression -> type == TYPE_CYCLE){
    @<Path Expression: Choose Shape in Cyclical Join@>
  }
}
@
\fimcodigo
...
value only when we read it explicitly between braces, when we are in a
concatenation or when we are in the beginning of an iteration. In the
last two cases because we need to copy the \monoespaco{curl 1}
specifier. Then, when we are in the end of each iteration, we preserve
the specifier in these cases, otherwise we erase it:
...
After,when we need to choose the shape when joining the cyclic
extremities, we will take into account the stored value:
...
to check its value and copy it. Otherwise, we also check if we have a
concatenation as the last join and in this case we need to apply the
rule of copying the implicit \monoespaco{curl 1}:
...
different than zero. If it is equal zero, we use instead the
value \monoespaco{curl 1}. If a path is cyclic, the cyclic nature is
taken into account.
