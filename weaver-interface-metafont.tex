...
@<Expressão de Caminho: Itera sobre Junções@>=
{
  ...
  if(end_expression -> type == TYPE_CYCLE){
    @<Expressão de Caminho: Escolhe Forma da Junção Cíclica@>
  }
}
@
\fimcodigo
...
lemos ele diretamente por existir um especificador explícito, quando
estamos em junção de concatenação ou então quando estamos na primeira
iteração, nestes dois últimos casos para podermos copiar depois
o \monoespaco{curl 1}. Então, no fim da iteração que interpreta
junções, nós armazenamos o especificador anterior nestes casos. Nos
demais, nós só os apagamos:
...
extremidades cíclicas, vamos levar isso em conta:
...
copiá-lo se não existir um especificador explícito. Também aplicamos a
regra de copiar um \monoespaco{curl 1} implícito se a última junção
for uma concatenação:
...
e $z-v$ é diferente de zero. Se for igual a zero, assumimos
simplesmente o valor \monoespaco{curl 1}. Naturalmente, o caráter
cíclico do caminho deve ser levado em conta.
...
especificador com as regras que descrevemos:
