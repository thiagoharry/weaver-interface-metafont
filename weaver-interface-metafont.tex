...
struct variable *insert_variable(int nesting_level,
  ...
  switch(type){
    ...
    case TYPE_T_PEN:
      ...
      break;
    // XXX: TODO: pen, picture and boolean
    default:
...
\subsecao{7.3. Variáveis de Caneta}
...
struct pen_variable{
  ...
  bool convex, straight, circular; // Sobre o caminho
...
um conjunto de triângulos em um processo chamado de triangulação.

A variável \monoespaco{convex} armazena se estamos diante de um
...
A variável \monoespaco{straight} armazena se o formato de nossa caneta
...
A variável \monoespaco{circular} armazena se nós sabemos que a caneta
...
Além das variáveis de caneta defiidas pelo usuário, iremos suportar a
...
mf -> internal_pen_variables[1].convex = true; // A caneta 'pensquare'
...
if(mf -> internal_pen_variables[1].format == NULL){
  ...
  return NULL;
} // Alocado o formato da caneta quadrada
...
// Preenchendo abaixo as coordenadas dos vértices de 'pensquare'.
// Não precisamos preencher pontos de controle, como a marcamos como tendo
// formato reto, os pontos de controle serão ignorados.
...
if(!pen.straight && a.value != 1.0){ // Requer retriangular a caneta:
  if(pen.referenced != NULL){
...
if(!pen.straight && a.value != 0.0){ // Requer retriangular a caneta:
  if(pen.referenced != NULL){
    recursive_copy_points(temporary_alloc, &(pen.format),
                          pen.referenced -> format, true);
    pen.referenced = NULL;
