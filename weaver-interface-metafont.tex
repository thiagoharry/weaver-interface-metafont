...
símbolo de multiplicação, divisão, um token \monoespaco{length} ou
então quando antes dele há outro operador terciário. O código abaixo
também deve tratar tais exceções.
...
divisão. Entretanto, deve-se levar em conta que o token de divisão só
deve ser tratado neste estágio se aquilo que estiver antes e depois
dele não forem dois tokens numéricos. Pois se forem, estamso diante de
uma fração e elas são tratadas com um nível de precedência ainda
maior:
...
\subsubsecao{8.1.3. Módulo e Valores Intermediários}

As regras de expressões numéricas primárias é:

\alinhaverbatim
<Primário Numérico> -> <Átomo Numérico> |
                       <Átomo Numérico> [ <Expressão Numérica> ,
                                          <Expressão Numérica> ] |
...
significados se sucedido por outro tipo de expressão primária. Já o
operador usando colchetes significa:

$$
a[b, c] = a(b+c)
$$

Ele permite que escrevamos de maneira mais rápida que o meio do
caminho entre $a$ e $b$, ou seja, sua média, é $.5[a,b]$.
...
bool eval_numeric_primary(struct metafont *mf, struct context *cx,
  ...
  //@<Primário Numérico: Regra 5@>
  //@<Primário Numérico: Regra 6@>
  //@<Primário Numérico: Regra 7@>
  return true;
}
@
...
1) Se a expressão tem um único token, ela é um único Átomo
Numérico. Esse é o caso mais comum, então vamos tratá-lo primeiro e
mais rapidamente, sem a necessidade de chamar mais funções
recursivamente. Apenas copiamos o conteúdo presente referenciado pelo
token para o lugar reservado para colocar o resultado da expressão:
...
expressão depois dele é numérica, e se for, calculamos o seu módulo.

3) Se encontrarmos algum dos operadores numéricos, temos um operador
numérico seguido de um primário numérico.

4) Se tudo for apenas um token numérico e uma variável, também é um
operador numérico seguido por primário numérico (multiplicação escalar).

5) Se houver token numérico, ``/'', outro token numérico e uma
variável, também estamos diante de um operador numérico (multiplicação
escalar).

6) Se o último token for um ``]'', estamos diante do operador de valor
intermediário.

7) Em todos os outros casos, tudo é apenas um Átomo Numérico.

Implementando estas sete regras, temos:
