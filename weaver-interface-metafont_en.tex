...
<Direction Specifier> -> Empty | { curl <Numeric Expression> } |
                         { <Pair Expression } |
                         { <Numeric Expression> , <Numeric Expression> }
...
#define TYPE_CURL           57 // Symbolic token 'curl'
@
\fimcodigo
...
our new variable and how should we allocate space for it. If we have a
cyclic path (expression ending with \monoespaco{cycle} token), then
the number of points to be allocated is equal the number of
joins. Otherwise, the number of points is equal the number of joins
plus 1:
...
And we need to do the same with $z_2$, the other extremity:
...
(which is equivalent to have direction $(0, 0)$), they can be a pair
specifying a direction vector or they can be an integer specifying the
curl.

If we have a pair, we will store in the variables \monoespaco{(w0\_x,
w0\_y)} and \monoespaco{(w1\_x, w1\_y)}. If we have a curl, we will
store in variables \monoespaco{curl0} and \monoespaco{curl1}. We also
...
previous direction specifier now will store what in the previous
iteration was the second current specifier. The current specifiers are
set as null, except that if the previous iteration stored a next
specifier, this specifier is our first one and we do not need to read
it again. After this, the next value is erased. And finally, after
this, we readq the second direction specifier from the tokens:
...
Now we check if we have a curl specification:
...
float tension0 = NAN, tension1 = NAN, u_x = NAN, u_y = NAN, v_x = NAN,
      v_y = NAN;
bool atleast0, atleast1; // If our tension is just an inferior limit
@
\fimcodigo
...
Anf finally, if we are not in a cyclic path and we do not have a
