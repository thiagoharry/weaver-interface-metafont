...
\subsubsecao{8.1.3. Modulus, Intermediary Values,
...
2) If the last token is a ``]'', then we have a construction like
$a[b,c]$, where $a$ is a numeric atom while $b$ and $c$ are numeric
expressions. We should separate the tokens in these three parts,
discover the value of each one and compute $a(b+c)$.
...
3) If we find operator \monoespaco{length}, then we check if the
...
4) If we find some of the numeric operators, we have a numeric
...
5) In the remaining cases, we have a scalar multiplicationwhere the
